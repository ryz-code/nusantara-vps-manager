...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/ryz-code/nusantara-vps-manager/autoscript/izin/ip | grep $MYIP | awk '{print $2}')
...
cp -r /var/lib/ryzvpn-premium/ /root/backup/ryzvpn-premium" &> /dev/null
...
LLatest=`date`
Get_Data () {
git clone https://github.com/ryz-code/user-backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "ryjprjkt@gmail.com" &> /dev/null
    git config --global user.name "ryz-code" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M master &> /dev/null
    git remote add origin https://github.com/ryz-code/user-backup
    git push -f https://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@github.com/ryz-code/user-backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/ryz-code/user-backup/master/$NameUser/$NameUser.zip"
sleep 1
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}               • NUSANTARA VPS MANAGER •                 $COLOR1│"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo
