...
cekdata=$(curl -sS https://raw.githubusercontent.com/ryz-code/user-backup/master/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/ryz-code/user-backup/master/$NameUser/$NameUser.zip" &> /dev/null
...
sleep 1
cp -r /root/backup/nusantara-vps /var/lib/ &> /dev/null
cp -r /root/backup/.acme.sh /root/ &> /dev/null
cp -r /root/backup/nusantara /etc/ &> /dev/null
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • NUSANTARA VPS MANAGER •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
read -n 1 -s -r -p "   Press any key to back on menu"
menu-backup
