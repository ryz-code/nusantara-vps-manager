...
fun_botOnOff() {
    [[ $(ps x | grep "bot_plus" | grep -v grep | wc -l) = '0' ]] && {
        clear
        echo -e "\E[44;1;37mㅤ⭐ NUSANTARA VPS MANAGERㅤTELE BOT INSTALLERㅤ⭐ㅤ\E[0m\n"
        ...
        fun_bot1() {
        	[[ "$(grep -wc '16' /etc/issue.net)" != '0' ]] && {
        		wget -qO- https://raw.githubusercontent.com/ryz-code/nusantara-vps-manager/autoscript/bot/ShellBot.sh > /etc/VPSManager/ShellBot.sh
        	}
            [[ ! -e "/etc/nusantara/ShellBot.sh" ]] && {
				wget -qO- https://raw.githubusercontent.com/ryz-code/nusantara-vps-manager/autoscript/bot/ShellBot.sh >/etc/VPSManager/ShellBot.sh
			}
            cd /etc/nusantara
        ...
        fun_bar 'fun_bot1'
        [[ $(ps x | grep "bot_plus" | grep -v grep | wc -l) != '0' ]] && echo -e "\n\033[1;32m⭐ NUSANTARA VPS MANAGERㅤTELE BOT ENABLED!ㅤ⭐\033[0m" || echo -e "\n\033[1;31m◇ MISTAKE! REANALYZE YOUR INFORMATION\033[0m"
        sleep 2
        menu
    } || {
        clear
        echo -e "\033[1;32m⭐ NUSANTARA VPS MANAGERTELE BOT STOPPING...ㅤ⭐\033[0m\n"
        fun_bot2() {
...
[[ -f "/etc/nusantara/ShellBot.sh" ]] && fun_botOnOff || fun_instbot
#fim
