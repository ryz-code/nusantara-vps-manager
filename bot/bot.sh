...
get_botName=$(sed -n '1 p' /root/nusantara/bot.conf | cut -d' ' -f2)
get_limituser=$(sed -n '2 p' /root/nusantara/bot.conf | cut -d' ' -f2)
res_price=5

ShellBot.init --token $get_Token --monitor --return map --flush --log_file /root/log_bot

msg_welcome() {
    oribal=$(grep ${message_from_id} /root/nusantara/reseller | awk '{print $2}')
    if [ "${message_from_id[$id]}" == "$get_AdminID" ]; then
        ...
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            ...
            --parse_mode html
    elif [ "$(grep -wc ${message_from_id} /root/nusantara/reseller)" != '0' ]; then
        local msg
...
backReq() {
    oribal=$(grep ${callback_query_from_id} /root/nusantara/reseller | awk '{print $2}')
    if [ "${callback_query_from_id[$id]}" == "$get_AdminID" ]; then
        ...
        ShellBot.editMessageText --chat_id ${callback_query_message_chat_id[$id]} \
            ...
            --reply_markup "$keyboard1" \
            --parse_mode html
    elif [ "$(grep -wc ${callback_query_from_id} /root/nusantara/reseller)" != '0' ]; then
        local msg
        msg="Welcome Reseller\n\n"
...
freeReq() {
    if [ "$(cat /root/nusantara/public)" == "on" ]; then
        local msg
...
publicReq() {
    limituser=$(sed -n '2 p' /root/nusantara/bot.conf | cut -d' ' -f2)
    if [ "$(cat /root/nusantara/public)" == "off" ]; then
        echo "on" >/root/nusantara/public
        echo "" >/root/nusantara/claimed
        ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
            --text "✅ Public Mode Is On, Limit Is $limituser ✅"
    else
        echo "off" >/root/nusantara/public
        ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
...
req_voucher() {
    ...
    if [ "$(grep -wc $coupon /root/nusantara/voucher)" != '0' ]; then
        echo "voucher"
...
req_url() {
    ori=$(grep ${callback_query_from_id} /root/nusantara/reseller | awk '{print $2}')
...
link_voucher() {
    ...
    exp=$(grep $vouch /root/nusantara/voucher | awk '{print $2}')
...
req_limit() {
    ...
    coupon=$(grep 'start [^_]*' $file_user | grep -o '[^_]*' | cut -d' ' -f2 | sed -n '3p')
    ori=$(grep ${message_from_id} nusantara/reseller | awk '{print $2}')
    if [ "${message_from_id[$id]}" == "$get_AdminID" ]; then
        echo ""
    elif [ "$(grep -wc ${message_from_id} /root/nusantara/reseller)" != '0' ]; then
        echo "reseller"
    elif [ "$(grep -wc $coupon /root/nusantara/voucher)" != '0' ]; then
        echo "voucher"
    elif [ "$(grep -wc ${message_from_id[$id]} /root/nusantara/claimed)" != '0' ]; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "Already Redeem" \
            --parse_mode html
        exit 1
        ...
        echo "${message_from_id[$id]}" >>/root/nusantara/claimed
    fi
}
...
allRes() {
    result=$(cat /root/nusantara/reseller)
    ShellBot.editMessageText --chat_id ${callback_query_message_chat_id[$id]} \
        --message_id ${callback_query_message_message_id[$id]} \
        --text "━━━━━━━━━━━━━━━━━━━━━\n 🟢 Reseller 🟢 \n━━━━━━━━━━━━━━━━━━━━━\n\n$result\n\n━━━━━━━━━━━━━━━━━━━━━\n" \
        --reply_markup "$keyboard6" \
...
reseller_input() {
    ...
    if [ "$(grep -wc ${User} /root/nusantara/reseller)" = '0' ]; then
        echo "$User $Balance" >>/root/nusantara/reseller
        local msg
...
balance_reseller() {
    ...
    User=$(sed -n '1 p' $file_user | cut -d' ' -f1)
    Balance=$(sed -n '2 p' $file_user | cut -d' ' -f1)
    ori=$(grep ${User} /root/nusantara/reseller | awk '{print $2}')
    topup=$(($ori + $Balance))
    sed -i "/${User}/d" /root/nusantara/reseller
    echo "${User} $topup" >>/root/nusantara/reseller
    if [ "$(grep -wc ${User} /root/nusantara/reseller)" != '0' ]; then
        local msg
...
reseller_balance() {
    ori=$(grep ${message_from_id} /root/nusantara/reseller | awk '{print $2}')
    if [ "$(grep -wc ${message_from_id} /root/nusantara/reseller)" != '0' ]; then
        if (($ori < $res_price)); then
            ...
            topup=$(($ori - $res_price))
            sed -i "/${message_from_id}/d" /root/nusantara/reseller
            echo "${message_from_id} $topup" >>/root/nusantara/reseller
        fi
    else
        echo "admin"
    fi
...
req_ovpn() {
    ...
    coupon=$(grep 'start [^_]*' $file_user | grep -o '[^_]*' | cut -d' ' -f2 | sed -n '3p')
    masaaktif=$(grep $coupon /root/nusantara/voucher | awk '{print $2}')
    ...
    sed -i "/$coupon/d" /root/nusantara/voucher
}
...
create_vmess() {
    ...
    coupon=$(grep 'start [^_]*' $file_user | grep -o '[^_]*' | cut -d' ' -f2 | sed -n '3p')
    expadmin=$(grep $coupon /root/nusantara/voucher | awk '{print $2}')
    req_voucher $file_user
    req_limit
    ...
    domain=$(cat /root/domain)
    nusantara="$(cat ~/log-install.txt | grep -w "VLess TCP XTLS" | cut -d: -f2 | sed 's/ //g')"
    none=80
    email=${user}
...
create_vless() {
    ...
    vlesslink1="vless://$uuid@$domain:$nusantara?encryption=none%26security=tls%26sni=%26type=ws%26host=%26path=/xrayws#$user"
...
create_xtls() {
    ...
    splice="vless://$uuid@$domain:$nusantara?flow=xtls-rprx-splice%26encryption=none%26security=xtls%26sni=%26type=tcp%26headerType=none%26host=#$user"
    direct="vless://$uuid@$domain:$nusantara?flow=xtls-rprx-direct%26encryption=none%26security=xtls%26sni=%26type=tcp%26headerType=none%26host=#$user"
    cat <<EOF >>"/etc/scvpn/config-user/${user}"
${splice}
${direct}
EOF
...
create_trojan() {
    ...
    tro="trojan://$uuid@$domain:$nusantara?sni=#$user"
    cat <<EOF >>"/etc/scvpn/config-user/${user}"
${tro}
EOF
    systemctl restart xray
...
input_voucher() {
    ...
    if [ "$(grep -wc $voucher /root/nusantara/voucher)" != '0' ]; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "Valid Voucher\n" \
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                'OVPN (USER EXPIRED) :')
                    ...
                    echo "$vouch $exp" >>/root/nusantara/voucher
                    local msg
                    msg="User : $user\n"
                    msg+="<code>Expired : $exp</code>\n\n"
                    msg+="https://t.me/${get_botName}?start=ovpn_${user}_${vouch}\n"
                ...
                'Change Limit:')
                    ...
                    sed -i "/Limit/d" /root/nusantara/bot.conf
                    echo "Limit: $freelim" >>/root/nusantara/bot.conf
                    local msg
